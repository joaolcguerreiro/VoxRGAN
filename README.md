...
# <span style="color: orange">VoxRGAN</span>

Official repository for VoxRGAN, a generative adversarial network for 3D medical image reconstruction. 

![VoxRGAN qualitative results across every training epoch](./figures/sample.gif)
...
#### Reconstruction
   ...
   model = torch.hub.load(
      'joaolcguerreiro/VoxRGAN', 'load_model',
...
1) Clone GitHub repository:
   ...
   ```
   git clone https://github.com/joaolcguerreiro/VoxRGAN.git
   cd VoxRGAN
   ```

2) Create environment:
   ```
   conda create -n voxrgan python=3.10.11
   conda activate voxrgan
   ```
